...
// `service` = celui de l'article (peut différer du service actif en multi-services)
export async function fetchTouretsForRef(cableId, service, magasin) {
  const { data: tc, error } = await supabase
    ...
    .eq('id', cableId).eq('service_id', service).eq('magasin_id', magasin)
    .maybeSingle();
...
// cart = [
//   { ref, nom, type: 'conso', qty, service_id },
//   { ref, nom, type: 'cable', qty, touretId, touretRef, service_id }
// ]
export async function validateSortie({ cart, magasin, userId, note = '' }) {
  if (!cart || cart.length === 0) return { ok: false, error: 'Panier vide' };

  const checks = [];
  for (const item of cart) {
    const itemService = item.service_id;
    if (!itemService) return { ok: false, error: `Service manquant pour ${item.nom || item.ref}` };
    if (item.type === 'conso') {
      const { data: art, error } = await supabase.from('articles_conso')
        .select('id, qty, nom').eq('ref', item.ref)
        .eq('service_id', itemService).eq('magasin_id', magasin).maybeSingle();
      ...
      const { data: t, error } = await supabase.from('tourets')
        .select('id, ref_touret, restante, initiale, type_cable_id').eq('id', item.touretId).maybeSingle();
  ...
  for (const item of checks) {
    if (item.type === 'conso') {
      ...
      mouvementsToLog.push({
        type: 'sortie', service_id: item.service_id, magasin_id: magasin,
        ref: item.ref, nom: item.nom, qty: -item.qty, user_id: userId, note,
      });
    } else {
      ...
      mouvementsToLog.push({
        type: 'sortie', service_id: item.service_id, magasin_id: magasin,
        ref: item.ref, nom: item.nom, qty: -item.qty, user_id: userId, note: noteEnrichie,
      });
    }
  }
